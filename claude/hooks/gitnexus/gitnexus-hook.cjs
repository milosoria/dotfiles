...
function handlePostToolUse(input) {
  ...
  try {
    ...
    /* no meta — never indexed */
  }

  // Never-indexed repo: stay silent. Only nudge where an index already
  // exists and is worth keeping fresh.
  ...
  sendHookResponse(
    'PostToolUse',
    `GitNexus index is stale (last indexed: ${lastCommit.slice(0, 7)}). ` +
